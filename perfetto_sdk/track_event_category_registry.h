...
struct TypedCategorySearchResult {
  ...
  static size_t GlobalCategoryId() {
    return type::kGlobalCategoryIds[index];
  }
};
...
static constexpr size_t FindCategoryInSlot(const char *name, int index = 0) {
  return (index == tag_type::kCategoryCount)
             ...
             : (StringEq(tag_type::kCategories[index].name, name)
                   ? index : FindCategoryInSlot<tag_type>(name, index + 1));
}

template<size_t v>
struct SizeType { static constexpr size_t value = v; };
...
struct FindNextUndefinedSlot<UniqueType, slot, 0 * sizeof(LocalSlotIdCounter<slot>)> : FindNextUndefinedSlot<UniqueType, slot+1> { };
...
struct GetCategoryIdImpl<UniqueType, slot_number, 0 * sizeof(LocalSlotIdCounter<slot_number>)> {
  static constexpr CategorySearchResult Get(const char *name) {
    return GetHelper(FindCategoryInSlot<typename LocalSlotIdCounter<slot_number>::type>(name), name);
  }
  ...
  static constexpr CategorySearchResult GetHelper(size_t index, const char *name) {
...
struct RegisterOnConstructor {
  RegisterOnConstructor(const Category* category_list, size_t* global_category_ids, size_t len);
};

} // namespace internal
} // namespace perfetto

#define PERFETTO_DECLARE_CATEGORIES_IMPL(tag_type, tag_slot_number_type, unique_type, ...) \
  namespace perfetto {                                                         \
  namespace internal {                                                         \
    struct unique_type; \
    struct tag_slot_number_type { \
      static constexpr size_t slot_number = FindNextUndefinedSlot<unique_type>::value; \
    }; \
    struct tag_type { \
...
#define PERFETTO_DECLARE_CATEGORIES(tag_type, ...)                          \
  PERFETTO_DECLARE_CATEGORIES_IMPL(perfetto_catg_decl_ ## tag_type, \
    perfetto_catg_decl_tag_slot_ ## tag_type, \
    perfetto_catg_decl_unique_type_ ## tag_type, \
    __VA_ARGS__)
...
#define PERFETTO_DEFINE_CATEGORIES_IMPL(tag_type, tmp_var) \
  ...
  namespace internal {                                                         \
    ...
    static ::perfetto::internal::RegisterOnConstructor tmp_var( \
      &tag_type::kCategories[0], &tag_type::kGlobalCategoryIds[0], \
