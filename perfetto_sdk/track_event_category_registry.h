...
template <size_t slot_number> struct CategoryDeclarationSlot;
...
template <typename UniqueType, size_t slot, size_t = 0> struct FindCategory {
...
struct FindCategory<UniqueType, slot,
                    0 * sizeof(CategoryDeclarationSlot<slot>)> {
  ...
  static constexpr size_t Find(const char *name, int index = 0) {
    return (index == CategoryDeclarationSlot<slot>::kCategoryCount)
               ...
               : StringEq(
                     CategoryDeclarationSlot<slot>::kCategories[index].name,
                     name)
...
struct GetCategoryIdImpl {
  ...
  static constexpr size_t GetHelper(size_t id, const char *name) {
    return id != kInvalidCategoryIndex
               ? id
...
using RegistrySlotsType =
    TrackEventCategoryRegistry[1 + kMaxCategorySlotNumber];

RegistrySlotsType &GetRegistrySlots();

template <size_t category_id>
constexpr const Category &GetCategoryAtCompileTime() {
  return CategoryDeclarationSlot<category_id & kMaxCategorySlotNumber>::
      kCategories[category_id >> kNumBitsRequiredForCategorySlotNumber];
}
...
struct CategoryDeclarationOnConstructor {
  CategoryDeclarationOnConstructor(size_t slot_number,
                                   TrackEventCategoryRegistry decl);
};
...
#define PERFETTO_DEFINE_CATEGORIES(slot_number)                                \
  namespace perfetto {                                                         \
  namespace internal {                                                         \
  const ::perfetto::Category                                                   \
      CategoryDeclarationSlot<slot_number>::kCategories[];                     \
  namespace {                                                                  \
  ::perfetto::internal::CategoryDeclarationOnConstructor                       \
      category_decl_tmp_var(                                                   \
          slot_number, ::perfetto::internal::TrackEventCategoryRegistry{       \
                           &::perfetto::internal::CategoryDeclarationSlot<     \
                               slot_number>::kCategories[0],                   \
                           ::perfetto::internal::CategoryDeclarationSlot<      \
                               slot_number>::kCategoryCount,                   \
                           __FILE__});                                         \
  }                                                                            \
  }                                                                            \
  }
